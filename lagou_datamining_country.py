...
def crawl():
	...
	loginheaders = {'Accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,*/*;q=0.8',
				...
				'User-Agent': headers}
	pn = 1
	url = 'http://www.lagou.com/jobs/positionAjax.json?px=default&first=true&kd=数据挖掘&pn='
	# 查看结果数量确定循环总页数
	init_url = url + str(pn)
...
def savefile(info):
	# 结果写入txt文档
	with open('lagou_datamining_country.txt', 'a') as f:
		for item in info:
