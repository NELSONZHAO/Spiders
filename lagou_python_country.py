# coding: utf-8
# 说明:爬取拉勾网中全国范围内所有含有Python关键词的职位
...
def crawl():
	...
	loginheaders = {'Accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,*/*;q=0.8',
				...
				'User-Agent': headers}
	pn = 1
	url = 'http://www.lagou.com/jobs/positionAjax.json?px=default&first=true&kd=Python&pn='
	# 查看结果数量确定循环总页数
	init_url = url + str(pn)
...
def savefile(info):
	# 结果写入txt文档
	with open('lagou_python_country.txt', 'a') as f:
		for item in info:
...
def savedb(info):
	...
	cursor = conn.cursor()
	sql = 'INSERT lagou_python_country(companyid, companyfullname, companyshortname, financestage, companysize, industry,' \
		  'city, district, businesszones, positionname, positionid, education, experience, jobnature, labels,' \
